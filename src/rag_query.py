...
from src.db_helper import DBHelper # added: for potential DB interactions
...
class RAGQuery:
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        # added: initialize db helper for email metadata lookups
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[{
                    ...
                    - For deadline/due date queries: include terms like "due date", "deadline", "submission", "assignment", "homework", "project due"
...
Otherwise, classify as "general"."""
    ...
    def _get_intent_routing_strategy(self, intent_info: Dict) -> Dict:
        ...
        strategies = {
            ...
            "deadline": {
                ...
                "prioritize_attachments": True,  # deadlines often in attachments (PDFs, docs)
    ...
    def _query_with_predict(self, query: str, top_k: int) -> Dict:
        ...
        # define stop words at the beginning so it's always available
...
Answer:"""
        ...
        try:
         ...
         # format sources - look up actual email metadata
        sources = []
        for event in events[:top_k]:
            message_ids = event.get("message_ids", [])
            message_id = message_ids[0] if message_ids else None
            
            # Try to get email metadata from database
            email_meta = None
            if message_id:
                email_meta = self.db_helper.get_email_metadata(message_id)
            
            # Use email metadata if available, otherwise fall back to event data
            if email_meta:
                sources.append({
                ...
                # Fallback: use event data if email not found in database
                # This can happen if the email was deleted or not yet processed
                sources.append({
...
Output ONLY the refined query, nothing else."""
    ...
    def _get_date_filter_for_query(self, query: str) -> Optional[Dict]:
        ...
        # For deadline queries, don't filter by email date - we care about deadline dates in content
        # Deadlines are often mentioned in older emails, so filtering by email date would exclude them
        if "deadline" in query_lower or "due date" in query_lower or "due dates" in query_lower:
            # Use a lenient filter (last 6 months) to avoid excluding relevant emails
            # but still filter out very old emails that are unlikely to be relevant
            six_months_ago = self.current_date - timedelta(days=180)
            min_timestamp = int(six_months_ago.timestamp())
