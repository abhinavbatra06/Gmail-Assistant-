...
Return ONLY valid JSON object with "events" array, no other text:"""
    ...
    def query_events(self, 
        ...
        if start_date:
            # For upcoming events: event must start on or after start_date
            # If start_time is NULL, exclude it (can't determine if it's upcoming)
            query += " AND e.start_time IS NOT NULL AND e.start_time >= ?"
            params.append(start_date)
        
        if end_date:
            # For date range: event must start before or at end_date
            # If start_time is NULL, exclude it
            query += " AND e.start_time IS NOT NULL AND e.start_time <= ?"
    ...
    def search_events(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        Returns:
            List of matching events with message_ids
        """
        cur = self.conn.cursor()
        search_term = f"%{query}%"
        
        # Include message_ids like query_events does
        cur.execute("""
            SELECT DISTINCT e.*, 
                   GROUP_CONCAT(DISTINCT ee.message_id) as message_ids,
                   COUNT(DISTINCT ee.message_id) as email_count
