...
class DBHelper:
    ...
    def get_email_metadata(self, msg_id):
        """
        Get email metadata (sender, subject, date) by message ID.
        
        Args:
            msg_id: Email message ID
            
        Returns:
            Dict with sender, subject, date, or None if not found
        """
        cur = self.conn.cursor()
        cur.execute("SELECT sender, subject, date FROM emails WHERE id=?", (msg_id,))
        row = cur.fetchone()
        if row:
            return {
