...
class EmailChunker:
    ...
    def chunk_single_email(self, message_id):
        ...
        if attachments:
            ...
            for i, attachment in enumerate(attachments, 1):
                ...
                # added: unique chunk IDs for attachments
                att_chunks = self._chunk_attachment(email_data, attachment, attachment_index=i-1)  # i-1 because i starts at 1
    ...
    # modified for unique chunk IDs
    def _chunk_attachment(self, email_data, attachment, attachment_index=0):
        """
        ...
        Args:
            ...
            attachment_index: Index of this attachment in the attachments list (for unique chunk IDs)
        ...
        if att_metadata.get("is_calendar") and attachment.get("events"):
            ...
            return self._chunk_calendar_events(email_data, attachment, metadata, attachment_index) # modified for unique IDs
        ...
        for para in paragraphs:
            ...
            if estimated_tokens > self.chunk_size and current_chunk_text:
                chunk = self._create_chunk(
                    ...
                    attachment_index=attachment_index  # modified for unique IDs
                )
                chunks.append(chunk)
                chunk_index += 1
    ...
    # added: event-aware chunking method, unique chunk IDs
    def _chunk_calendar_events(self, email_data, attachment, metadata, attachment_index=0):
        """
        ...
        for event_idx, event in enumerate(events):
            ...
            chunk = self._create_chunk(
                ...
                attachment_index=attachment_index  # for unique chunk IDs
            )
            chunks.append(chunk)
    ...
    def _create_chunk(self, text, chunk_index, source_type, email_data, 
                     metadata, attachment_name=None, event_data=None, attachment_index=None): # modified for unique IDs
        """
        ...
        Args:
            ...
            attachment_index: Index of attachment in attachments list (for unique IDs) # added for unique IDs
        ...
        # added
            ...
            if event_uid:
                # Include attachment_index to ensure uniqueness even if same event appears in multiple attachments
                if attachment_index is not None:
                    chunk_id = f"{message_id}_event_{event_uid}_att{attachment_index}_{chunk_index}"
                else:
                ...
                # Include attachment_index to ensure uniqueness
                if attachment_index is not None:
                    chunk_id = f"{message_id}_event_{clean_name}_att{attachment_index}_{chunk_index}"
                else:
            ...
            # Include attachment_index to ensure uniqueness even if same filename appears multiple times
            if attachment_index is not None:
                chunk_id = f"{message_id}_att{attachment_index}_{clean_name}_{chunk_index}"
            else:
